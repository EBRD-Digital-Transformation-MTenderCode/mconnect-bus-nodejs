      ...
      describe('Response from the treasury present', () => {
        ...
        describe('Fetch contract validation', () => {
          it('Should stop contract registration if registration failed', async () => {
            (fetchContractRegister as jest.Mock).mockImplementation(async () =>
          ...
          describe('Contract timestamp update', () => {
            describe('When contract exists in database', () => {
              ...
              it('Should terminate registration when timestamp already exists', async () => {
                ...
                expect(logger.error).toHaveBeenCalledWith(`🗙 Error in REGISTRATOR. Contract exists: Failed to update timestamp in treasuryRequests table for id_doc ${contract.id_doc}. Column timestamp seems to be already filled.`);
              });
            });
            ...
            describe('When contract does not exist in database', () => {
              ...
              it('Should terminate registration when timestamp already exists', async () => {
                ...
                expect(logger.error).toHaveBeenCalledWith(`🗙 Error in REGISTRATOR. Contract does not exist: Failed to update timestamp in treasuryRequests table for id_doc ${contract.id_doc}. Column timestamp seems to be already filled.`);
              });
            });
          })
        });
