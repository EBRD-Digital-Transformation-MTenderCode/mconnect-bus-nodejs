...
export type TInsertToTreasureResponses = (
  row: ITreasuryResponsesRow
) => Promise<null>;

const insertToTreasureResponses: TInsertToTreasureResponses = ({
  id_doc,
  status_code,
  message,
  ts_in
  ...
  const query = `
    ...
    VALUES ('${id_doc}', '${status_code}', '${JSON.stringify(
    message
  )}', to_timestamp(${tsToPgTs(+ts_in)}))
  `;
